...
roblox-studio-axi session start --clients <1..8>
...
Session commands also accept `--timeout <seconds>`. They intentionally reject `--studio`, `--force`, `--adopt`, and `--kill`; cleanup authority comes from the exact Local Server ownership tuple rather than caller selection. See [Managed Local Multiplayer Sessions](docs/local-multiplayer-sessions.md).
...
| 8 | cleanup failure |
| 9 | managed-session conflict |
| 10 | managed-session transaction busy |
| 11 | managed-session recovery required |
| 12 | interrupted |
| 13 | unsupported platform or control surface |
...
Managed session start/stop evidence is separate and user-local under `%LOCALAPPDATA%\roblox-studio-axi\sessions\v1\evidence`. It records normalized observations and ordered mutation actions; status writes no evidence.
...
- Generic play mutations run through a guard that fails closed beside any Managed Session Record or plausible multiplayer topology.
- Managed cleanup calls server-side `StudioTestService:EndTest()` only after a fresh exact ownership proof and removes its record only after verified teardown.
